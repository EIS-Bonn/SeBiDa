...
import bde.sebida.model.Triple;
import scala.Tuple2;
...
public class Loader implements Serializable {
    ...
    public void fromSemData(String input_path, final String output_path, String dsName, String dsIRI, String master) throws IOException, ClassNotFoundException {
        ...
        try {
            ...
            /*JavaRDD<Row> triples = spark.read().format("com.databricks.spark.csv").option("header","false").option("quote",'"').csv(input_path).toJavaRDD();

            // 3. Map Triple objects to pairs (Triple.subject,[Triple.property, Triple.object])
            @SuppressWarnings({ "rawtypes" })
            JavaPairRDD<String, Tuple2<String, String>> subject_property = triples.mapToPair(new PairFunction<Row, String, Tuple2<String, String>>() {

                @Override
                public Tuple2<String, Tuple2<String, String>> call(Row trpl) throws Exception {
                    return new Tuple2(trpl.get(0), new Tuple2(trpl.get(1), trpl.get(2)));
                }
            });*/
                    ...
                    public Row call(Tuple2<String, Iterable<Tuple2<String, String>>> i) throws Exception {
                        ...
                        while (iter.hasNext()) {
                            ...
                            if(readColumns.contains(property + "--TD") && !readColumns.contains(property + "--TAD")) {
                                ...
                                ArrayList<String> temparr = null; // In new Parquet, ArrayString type saves only String[]s not ArrayLists, so needs to change back and forth from String to ArrayList String
                                String[] arr = null;
